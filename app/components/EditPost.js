...
function EdidPost(props) {
    ...
    switch (action.type) {
      ...
      case "bodyRules":
        if(!action.value.trim()) {
          ...
          draft.body.message = "You must provide a body Text."
        }
        return 
      case "notFound":
        draft.notFound = true
      ...
      try {
        ...
        if(response.data) {
          ...
          if (appState.user.username !=response.data.author.username) {
            appDispatch({type: "flashMessage", value: "You do not have the permission to edit this post"})
  ...
  return (
    <Page title="Edit Post" >
      <Link className="small font-weight-bold" to={`/post/${state.id}`}>&laquo; Back to permalink</Link>
...
export default withRouter(EdidPost)
